...
object Tests
{
	...
	def makeParallel(loader: ClassLoader, runnables: Iterable[TestRunnable], setupTasks: Task[Unit], tags: Seq[(Tag,Int)]): Task[Map[String,TestEvent]] =
		toTasks(loader, runnables.toSeq, tags).dependsOn(setupTasks)

	def toTasks(loader: ClassLoader, runnables: Seq[TestRunnable], tags: Seq[(Tag,Int)]): Task[Map[String, TestEvent]] = {
		val tasks = runnables.map { case (name, test) => toTask(loader, name, test, tags) }
		tasks.join.map( _.foldLeft(Map.empty[String, TestEvent]) { case (sum, e) =>  
			sum ++ e		  
		} )
	}

	def toTask(loader: ClassLoader, name: String, fun: TestFunction, tags: Seq[(Tag,Int)]): Task[Map[String, TestEvent]] = {
	...
	def makeSerial(loader: ClassLoader, runnables: Seq[TestRunnable], setupTasks: Task[Unit], tags: Seq[(Tag,Int)]): Task[List[(String, TestEvent)]] = 
	{
		@tailrec
		def processRunnable(runnableList: List[TestRunnable], acc: List[(String, TestEvent)]): List[(String, TestEvent)] = 
			runnableList match {
				case hd :: rst => 
					val testFun = hd._2
	...
	def showResults(log: Logger, results: (TestResult.Value, Map[String, TestEvent]), noTestsMessage: =>String, summaries: Iterable[Summary]): Unit =
	{
		val multipleFrameworks = summaries.size > 1
		...
		for ((name, messages) <- summaries)
			printSummary(name, messages)
		val noSummary = summaries.headOption.forall(_._2.size == 0)
