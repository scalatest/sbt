...
public class ForkMain {
	...
	private static class Run {
		...
		ForkEvent[] runTest(ForkTestDefinition test, Runner runner, Logger[] loggers, ObjectOutputStream os) {
			...
			if (runner instanceof RunnerWrapper)
				((RunnerWrapper) runner).task(test.name, test.fingerprint).execute(handler, loggers);
			else {
				boolean isModule;
				if (test.fingerprint instanceof SubclassFingerprint) {
					isModule = ((SubclassFingerprint) test.fingerprint).isModule();
				} else {
					isModule = ((AnnotatedFingerprint) test.fingerprint).isModule();
				}
				// TODO: To pass in correct explicitlySpecified and selectors
				runner.task(test.name, isModule, false, new Selector[] { new SuiteSelector() }).execute(handler, loggers);
			}
