...
final class TestEvent(val result: TestResult.Value, val passedCount: Int, val failureCount: Int, val errorCount: Int, val skippedCount: Int)
object TestEvent
{
	def apply(events: Seq[TEvent]): TestEvent =
	{
		...
		new TestEvent (overallResult, count(TStatus.Success), count(TStatus.Failure), count(TStatus.Error), count(TStatus.Skipped))
	}
	val Error: TestEvent = new TestEvent(TestResult.Error, 0, 0, 0, 0)
	val Empty: TestEvent = new TestEvent(TestResult.Passed, 0, 0, 0, 0)
}
