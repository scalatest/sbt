...
package sbt
	...
	import org.scalasbt.testing.{AnnotatedFingerprint, Fingerprint, SubclassFingerprint, DoNotDiscoverFingerprint}
	import org.scalasbt.testing.{Event, EventHandler, Framework, Runner, Logger=>TLogger, Task => TestTask}
...
final class TestRunner(delegate: Runner, listeners: Seq[TestReportListener], log: Logger) {
	...
	final def run(testDefinition: TestDefinition, testTask: TestTask): (TestResult.Value, Array[TestTask]) =
	{
		...
		def runTest() =
		{
			...
			(event.result.getOrElse(TestResult.Passed), nestedTasks)
		}

		safeListenersCall(_.startGroup(name))
		try
		{
			val (result, nestedTasks) = runTest()
		...
		catch
		{
			case e: Throwable =>
				...
				(TestResult.Error, Array.empty[TestTask])
		}
	}
...
{ 
	...
	{
		val interfaceJar = IO.classLocationFile(classOf[org.scalasbt.testing.Framework])
...
abstract class TestFunction(val testDefinition: TestDefinition, val runner: TestRunner, fun: (TestRunner) => (TestResult.Value, Array[TestTask])) extends Function0[(TestResult.Value, Array[TestTask])] {

	def apply(): (TestResult.Value, Array[TestTask]) = fun(runner)
