...
final class TestRunner(delegate: Runner, listeners: Seq[TestReportListener], log: Logger) {
	...
	final def run(testDefinition: TestDefinition, testTask: TestTask): (TestResult.Value, Seq[TestTask]) =
	{
		...
		def runTest() =
		{
			...
			(event.result.getOrElse(TestResult.Passed), nestedTasks.toSeq)
		}
		...
		catch
		{
			case e: Throwable =>
				...
				(TestResult.Error, Seq.empty[TestTask])
		}
	}
...
abstract class TestFunction(val testDefinition: TestDefinition, val runner: TestRunner, fun: (TestRunner) => (TestResult.Value, Seq[TestTask])) {

	def apply(): (TestResult.Value, Seq[TestTask]) = fun(runner)
